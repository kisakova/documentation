...
#. In OroCommerce, in **System > Localization > Languages** section, ensure that syncronization is enabled for the target language. `Contact Oro Translation Team if you Face any Issues`_ (e.g. your translation does not appear in the OroCommerce after it has been approved).
...
#. Sign in into your Crowdin account or sign up for a new user account if you don not have one yet:
   ...
   #) Sign in using your github account or social media account (facebook, google+, twitter). Alternatively, create a new crowdin account: enter your email, username, password and password confirmation, and click **Create account**. Follow the link in the confirmation Email to activate you account.
...
#. Select the target language for OroCommerce translation and click :guilabel:`Join` next to the following message that appears: “You must join the translators team to be able to participate in this project”.

#. Type in the reason why your are joining the translation team (e.g. as a developer, I'd like to enable Korean translation for OroCommerce and my customization) and click :guilabel:`Join`.

#. Your request will be reviewed by the Oro support team. Upon approval, you will get an email from Crowdin with invitation to start contibuting.
...
#. Select the yaml file with translations you would like to contribute to.
...
#. When the translation is approved, it is marked with a green check and moved to the end of the list on the translation page. Approved translations are merged (published) to the OroCommerce translations once a day and becomes available in OroCommerce on the **System > Configuration > Language Settings** page.
...
2. If your target language is not listed, click :guilabel:`Add language`, select target language from the list and click **Add language** in the box to confirm the action. This will trigger download of the translation files from the Crowdin project into OroCommerce.

3. If the status for your target language is *Disabled*, click on a **tick** icon in the **Updates** column. This will enable loading the translation updates automaticaly from the Crowdin project into OroCommerce.
